            ...
            if (!next) {
                next = function (err) {
                    if (err && err.isBoom) {
                        ...
                        return err ? reply(boom.unauthorized('Unauthorized')) : reply(request.user);
                    }
                };
            }
...
internals.allFailedFactory = function (request, reply, failures, options, callback) {
    return function allFailed() {
        ...
        return reply(boom.unauthorized('Unauthorized', internals.getChallenges(failures) || null));
    };
};
...
internals.attemptFactory = function (passport, request, reply, name, failures, allFailed, options, next, callback) {
    return function attempt(i, cb) {
        ...
        if (!prototype) {
            return next(new Error('No strategy registered under the name:' + layer));
        }
